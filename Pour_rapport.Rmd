...
AMR <- AMR %>%
  filter(population != "Caprin") %>%
  filter(population != "Ovin")
...
```{r,echo=FALSE}
label <- c("Veau", "Lapin", "Poulet", "Porc", "Dinde", "Volaille", "Veau", "EHPAD", "ES-hemoculture", "ES", "Porc", "Dinde", "Ville", "Cheval", "Volaille", "Chien", "Poulet", "Chat", "Bovin")

ggplot(AMR, aes(x = `valeur AMR`, y = reorder(population, `valeur AMR`, median), fill = famille2))+
  geom_boxplot()+
  labs(title = "Boxplot pour l'étude des dispersions des valeurs Pan-S",
       subtitle = "Comparaison des Pan-S entre animaux et humains",
       caption = "Données: Une observation correspond à une région (ou la donnée nationale) et une année - Anses\n Note de lecture : Les veaux ont la pan-sensibilité la plus basse, ie. ce sont les animaux dont la \n population bactérienne est la plus résistante.",
       y = NULL,
       x = "Valeur Pan-S",
       fill = "Famille")+
  theme_minimal()+
...
label <- c("Chien", "Veau", "Chat", "Dinde", "Bovin", "Cheval", "Volaille-abattoir", "Dinde-abattoir", "Lapin", "Porc", "Porc-abattoir", "Poulet-abattoir", "Veau-abattoir")
...
ggplot(AMR_perte_regionale_animal, aes(x = reorder(population, perte), y = perte)) +
  ...
  labs(title = "Boxplot de la perte d'effectif lors du passage de l'échelle nationale à régionale",
       x = NULL,
       subtitle = expression(paste("Perte calculée à partir de cette formule : (1 - ", Sigma, " effectif régional / effectif national) * 100")),
       y = "Taux d'effectif perdu",
       caption = str_glue("Données : Pan-S des populations animales - Anses\nNote de lecture : Par année, moins de 20% de l'information est perdue en analysant les Pan-S des chiens l'échelle régionale."))+
  scale_y_continuous(labels = scales::label_percent(scale = 100))+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  annotate("text", x = 12, y = 0.19, label = "Moyenne = 22.75%", colour = "darkgreen", size = 3.3)+
...
ggplot(table_join, aes(x = reorder(population, -taux), y = 100-taux)) +
  ...
  labs(title = "Variation de la fraction d'isolats bactériens pour lesquels la pan-sensibilité\n n'a pas pu être calculée au sein de chaque population étudiée",
       x = NULL,
       y = "Taux de perte",
       caption = str_glue("Données : population animale au niveau national - Anses\nNote de lecture : En moyenne, 13.2% des animaux étudiés n’ont pas de profil de pan-sensibilité calculable.\nEn rouge, on observe la moyenne au sein de chaque population."))+
...
ggplot(table, aes(x = reorder(population, -taux), y = 100-taux)) +
  ...
  labs(title = "Variation de la fraction d'isolats bactériens pour lesquels la pan-sensibilité n'a pas\npu être calculée au sein de chaque population étudiée",
       x = NULL,
       y = "Taux de perte",
       caption = str_glue("Données : population humaine au niveau régional - Anses\nNote de lecture : En moyenne, 6.4% des humains étudiés n'ont pas un profil de résistance pan-sensible.\nEn rouge, on observe la moyenne au sein de chaque population."))+
  scale_y_continuous(labels =scales::number_format(suffix=" %"),limits=c(0,100))+
  scale_x_discrete(labels = population)+
  theme_minimal()+
  annotate("text", x = 1.5, y = 7, label = paste0("Moyenne = ", round(100-moyenne, 1), "%"), colour = "darkgreen", size = 3.2)
```

```{r}
table_table_join <- rbind(table[,c(2, 6)], table_join[,c(2, 5)])

moyenne<-mean(table_table_join$taux,na.rm = TRUE) 

moyennes<-table_table_join%>%
  group_by(population)%>%
  mutate(moyenne_taux=mean(taux,na.rm=TRUE))%>%
  select(population,moyenne_taux)%>%
  unique()

ggplot(table_table_join, aes(x = reorder(population, -taux), y = 100-taux)) +
  geom_boxplot() +
  geom_hline(yintercept = 100 - moyenne, color = "darkgreen", linetype = "dashed", size = 0.8) +
  geom_point(data=moyennes,aes(x = population, y = 100 - moyenne_taux), color = "red", size = 3)+
  labs(title = "Variation de la fraction d'isolats bactériens pour lesquels la pan-sensibilité n'a pas\npu être calculée au sein de chaque population étudiée",
       x = NULL,
       y = "Taux de perte",
       caption = str_glue("Données : population humaine au niveau régional et animale au niveau national - Anses\nNote de lecture : En moyenne, 6.7% des espèces étudiés n'ont pas un profil de résistance pan-sensible.\nEn rouge, on observe la moyenne au sein de chaque population."))+
  scale_y_continuous(labels =scales::number_format(suffix=" %"),limits=c(0,100))+
...
carteAMR1<-R1%>%
  ...
  labs(title = "Pan-sensibilité de la population en ville en France en 2021",
...
carteAMR1_Guad<-R1_Guad%>%
  ...
  labs(title = "Guadeloupe",
       caption = str_glue(paste("Données : population en ville en France métropolitaine et Outre-mer dont les données sont disponibles - Anses.\nNote de lecture : En 2021, les populations en ville ont une pan-sensibilité comprise entre ",min,"% et",max,"%.")))+
...
carteAMR1<-R1%>%
  ...
  labs(#title = "Pan-sensibilité de la population bovine en France en 2021",
       fill = "Pan-sensibilité",
       caption = str_glue(paste("Données : les bovins en France métropolitaine \n(des régions (en gris et Outre-mer) sont indisponibles) - Anses.\nNote de lecture : La Normandie présente la pan-sensibilité la plus élevée chez les bovins : (",max(R1$`valeur AMR`,na.rm=TRUE),").")))+
  scale_fill_gradient(name = "Taux de Pan-Sensibilité", limits = bornes,low='darkblue',high='lightblue')
carteAMR1_Guad<-R1_Guad%>%
  ggplot() +
  geom_sf(aes(geometry=geometry,fill=`valeur AMR`))+
...
carteAMR1<-R1%>%
  ...
  theme(axis.text = element_blank(),
        ...
        panel.grid.minor = element_blank(), # on supprime la grille
        panel.background = element_blank()) + 
  labs(#title = "Pan-sensibilité des populations chiens et chats en France en 2021",
       fill = "Pan-sensibilité",
       caption = str_glue(paste("Données : La population des bovins en France métropolitaine\n (des régions (en gris et Outre-mer) indisponibles) - Anses.\nNote de lecture : En Auverge-Rhône-Alpes le taux de Pan-S des chiens et chats\n est le plus bas :", min(R1$`valeur AMR`,na.rm=TRUE), "% en 2021.")))+
  scale_fill_gradient(name = "Taux de Pan-Sensibilité", limits=bornes,low='darkblue',high='lightblue')
carteAMR1_Guad<-R1_Guad%>%
  ggplot() +
  geom_sf(aes(geometry=geometry,fill=`valeur AMR`))+
...
ggplot(AMR_nationale_animal[which(AMR_nationale_animal$famille2 == "animale-abattoir"),], aes(x = annee, y = `valeur AMR`, group = population, color = population)) +
  ...
  labs(title = "Évolution de la Pan-S des populations issues d'abattoir entre 2012 et 2021",
       x = NULL,
...
ggplot(AMR_nationale_animal[which(AMR_nationale_animal$famille2 == "animale"),], aes(x = annee, y = `valeur AMR`, group = population, color = population)) +
  geom_line(stat = "summary", fun = "mean") +
  geom_point(stat = "summary", fun = "mean", size = 2) +
  labs(title = "Évolution de la sensibilité des animaux étudiés par le dispositif Resapath",
       x = NULL,
       y = "Pan-sensibilité moyenne",
       color = "Population",
       caption = str_glue("Données : les populations animales Resapath - Anses\nNote de lecture : La population bovin-adulte a perdu 17% de sensibilité de 2012 à 2021.")) +
  theme_minimal()

ggplot(AMR_nationale_humain, aes(x = annee, y = `valeur AMR`, group = population, color = population)) +
  geom_line(stat = "summary", fun = "mean") +
  geom_point(stat = "summary", fun = "mean", size = 2) +
  labs(title = "La sensibilité des populations de la famille humaine au cours du temps",
       x = NULL,
       y = "Pan-sensibilité moyenne",
       color = "Population",
       caption = str_glue("Données : les populations humaines - Anses \nNote de lecture : La Pan-S des personnes en EHPAD a augmenté d'environ 20% entre 2012 et 2021.")) +
  theme_minimal()+
...
pvaleurs_anim<-c(signif(resultats_tests$veau$p.value,2),
                 ...
                 signif(resultats_tests$Porc$p.valueue,2),
...
```{r}
usage_animal$path <- ifelse(usage_animal$annee == 2012, "Début de l'étude", ifelse(usage_animal$annee == 2021, "Fin de l'étude", "Au milieu de l'étude"))
...
ggplot(data = usage_animal[which(usage_animal$population %in% c("Bovin", "Chats et chiens", "Chevaux", "Lapins", "Porcs", "Volailles")),], aes(x = `valeur AMR`, y = `valeur AMU`))+
  ...
  scale_color_manual(values = c("Au milieu de l'étude" = "black", "Début de l'étude" = "green", "Fin de l'étude" = "red"))+
  facet_wrap(~population, scales = "free_y")

usage_animal$path[which(usage_animal$population == "Bovin-abattoir" & usage_animal$annee == 2015)] <- "Début de l'étude"
usage_animal$path[which(usage_animal$population == "Volailles-abattoir" & usage_animal$annee == 2020)] <- "Fin de l'étude"

ggplot(data = usage_animal, aes(x = `valeur AMR`, y = `valeur AMU`))+
  geom_path(size = 0.8)+
  geom_point(aes(colour = path), size = 1.3)+
  theme_minimal()+
  theme(legend.position = "bottom")+
  labs(x = "Pan-S",
       y = "Usage d'antibiotiques",
       title = "Nuage de point de la Pan-S des animaux en fonction de leur usage d'antibiotiques",
       subtitle = "Les couleurs renseignent les premières et dernières valeurs pour chaque population",
       caption = "Données : Usage animaux - Anses\nNote de lecture : En 2012, les porcs avaient une valeur d'usage de 1 pour une pan-sensibilité d'environ 22.5%.",
       color = "Année du test de Pan-S")+
  scale_color_manual(values = c("Au milieu de l'étude" = "black", "Début de l'étude" = "green", "Fin de l'étude" = "red"))+
  facet_wrap(~population, scales = "free_y")
...
hist(usage_humain$`valeur AMR`[which(usage_humain$population == "EHPAD")], freq = FALSE, breaks = 30,, xlim = c(0, 1), xlab = "Pan-S", ylab = "Densité")
title(sub = "Note : Bêta(a, b) où a et b sont calculés à partir de la moyenne et de la variance empirique", cex.sub = 0.8)
...
ggplot(data = mes_predictions_humains, aes(x = estimation, y = `vraie valeur`))+
  geom_ribbon(aes(ymin = borne_inf, ymax = borne_sup), fill = "#397e8a", alpha = 0.1) +
  geom_point()+
  geom_abline(color = "#397e8a")+
...
ggplot(data = mes_predictions_animaux, aes(x = estimation, y = `vraie valeur`))+
  ...
  geom_point()+
  geom_abline(color = "#397e8a")+
  labs(title = "Les prédictions des pan-S animales pour 2022 et l'intervalle de confiance associé",
       subtitle = "Intervalle de confiance calculé à 95%",
