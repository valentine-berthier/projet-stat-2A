...
for (pop in unique(usage_animal$population)){
  ...
  print(pop)
  print(predictions)
  table_prediction <- cbind(pop, predictions * 100)
...
for (pop in unique(usage_humain$population)){
  ...
  predictions <- predict(model_beta, newdata = usage_prediction_filtre, se.fit=TRUE, interval = "prediction", level = 0.99)
  print(pop)
  table_prediction <- cbind(pop, unique(usage_prediction_filtre$region), predictions * 100)
...
colnames(mes_predictions_humains) <- c("population", "region", "estimation", "vraie valeur")
...
colnames(mes_predictions_animaux) <- c("population", "estimation", "vraie valeur")
