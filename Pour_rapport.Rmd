...
effectifs_totaux_animale$abattoir <- grepl("abattoir", effectifs_totaux_animale$population, ignore.case = TRUE)
...
ggplot(effectifs_totaux_animale, aes(x = reorder(population, taux_eff_perte), y = taux_eff_perte)) +
  geom_boxplot() +
  geom_hline(yintercept = mean(effectifs_totaux_animale$taux_eff_perte), color = "darkgreen", size = 0.8, linetype = "dashed") +
  labs(title = "Perte d'effectif lors du passage de l'échelle nationale à régionale",
       x = NULL,
       y = "Taux d'effectif perdu",
       caption = str_glue("Cohorte : population animale\nLecture : Entre 2012 et 2021, 50% des années ont fait face à une perte régionale d'au moins 12.5% pour la population des chats."))+
  scale_y_continuous(labels =scales::number_format(suffix=" %"),limits=c(0,100))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_x_discrete(labels = label)+
  annotate("text", x = 11, y = 20, label = "Mean = 23.54%", colour = "darkgreen", size = 3.3)
```
...
=======
```{r,echo=FALSE}
>>>>>>> 4e853533e0fe7f18b328807c3a6105b455610fe0
table_join<-table_join%>%
  mutate(taux=(eff_pansensible/eff_C3G)*100)

label <- c("Lapin", "Dinde", "Poulet", "Porc", "Bovin", "Cheval", "Chien", "Chat", "Veau")
...
ggplot(table_join, aes(x = reorder(population, taux), y = 100-taux)) +
  ...
  labs(title = "Variation de la fraction d'isolats bactériens pour lesquels la pan-sensibilité n'a pas\npu être calculée au sein de chaque groupe étudié",
       x = NULL,
       y = "Taux de perte",
       caption = str_glue("Cohorte : population animale au niveau National\nLecture : En moyenne, 87 % des animaux étudiés ont un profil de résistance pan-sensible."))+
  scale_y_continuous(labels =scales::number_format(suffix=" %"),limits=c(0,100))+
  scale_x_discrete(labels = label)+
  annotate("text", x = 8, y = 18.5, label = "Mean = 13.2%", colour = "darkgreen", size = 3.3)
```
