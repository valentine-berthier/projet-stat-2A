...
for(i in unique(AMR_animale$population)){
  AMR_animale[which(AMR_animale$population==i),] %>% 
    levene_test(`valeur AMR` ~ region)
}

temporaire <- AMR_animale %>% 
  filter(population != "Poule-poulet")

resultats_levene <- list()

# Boucle sur les populations uniques
for (i in unique(temporaire$population)) {
  # Filtrer les données pour la population actuelle
  data_population <- temporaire[temporaire$population == i, ]
  
  # Réaliser le test de Levene pour cette population
  test <- levene_test(`valeur AMR` ~ region, data = data_population)
  
  # Stocker le résultat dans la liste
  resultats_levene[[i]] <- test
}

# Afficher les résultats
print(resultats_levene)
...
#### 1 : Régression linéaire pour connaître la valeur de pan-S

```{r}

glm(AMR_animale$`valeur AMR` ~ AMR_animale$)
```
