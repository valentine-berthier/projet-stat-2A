...
carteAMR1<-R1%>%
  ...
  scale_fill_continuous(name = "Légende", limits = c(30, 70),low='darkblue',high='lightblue')
carteAMR1_Guad<-R1_Guad%>%
  ...
  scale_fill_continuous(name = "Légende", limits = c(30, 70), breaks = seq(30, 70, by = 10))
...
carteAMR1<-R1%>%
  ...
  labs(title = "Pan-sensibilité de la population chien/chat en France en 2021",
       fill = "Pan-sensibilité")+
  scale_fill_gradient2(name = "Légende", limits = c(30, 70),low='darkblue',high='lightblue')
carteAMR1_Guad<-R1_Guad%>%
  ggplot() +
  geom_sf(aes(geometry=geometry,fill=`valeur AMR`))+
  theme(axis.text = element_blank(),
...
carteAMR1_Mart<-R1_Mart%>%
  ...
  theme(axis.text = element_blank(),
        ...
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),legend.position = "none")+
  scale_fill_gradient2(name = "Légende", limits = c(30, 70))
carteAMR1_Mayotte<-R1_Mayotte%>%
  ggplot() +
  geom_sf(aes(geometry=geometry,fill=`valeur AMR`))+
  theme(axis.text = element_blank(),
...
for (pop in unique(usage_humain$population)){
  ...
  model_glm <- glm(data = usage_filtre, `valeur AMR` ~ `valeur AMU`, family = binomial(link = "logit"))
  print(pop)
  print(summary(model_glm))
...
for (pop in unique(usage_humain$population)){
  usage_filtre <- usage_humain[which(usage_humain$population == pop),]
  usage_prediction_filtre <- usage_prediction_humain[which(usage_prediction_humain$population == pop), c(2, 4)]
  model_beta <- betareg(data = usage_filtre, `valeur AMR` ~ `valeur AMU` + annee, link = "logit")
  predictions <- predict(model_beta, newdata = usage_prediction_filtre, se.fit=TRUE, interval = "prediction", level = 0.99)
  print(pop)
  print(predictions)
}
