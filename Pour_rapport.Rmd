...
ggpubr::ggqqplot(residuals(model))# Sur le qqplot, les points sont, en grande partie, alignés avec la première bissectrice, on suppose l'hypothèse de normalité vérifiée
...
Le test de Cochran Armitage est utilisé ici. Il serait intéressant de faire un test unilatéral droit plutôt que bilatéral, car ce qu'on souhaite c'est observer une baisse de la sensibilité (mais bon comme dit en réunion, c'est clairement pas ce qu'on voit sur nos graphiques).
