...
https://rdrr.io/cran/betareg/man/predict.betareg.html
...
### Données de 2022 pour la prédiction

```{r}
usage_prediction_animal <- read_excel("donnees/AMU2022.xlsx", sheet = "données animales nationales")
colnames(usage_prediction_animal) <- c("population", "annee", "valeur AMU", "valeur AMR", "effectif AMR")
usage_prediction_humain <- read_excel("donnees/AMU2022.xlsx", sheet = "données humaines régionales")

# Si le modèle contient le paramètre region, alors il faut supprimer les données pour lesquelles on ne peut pas faire de prédiction (Guyane, Mayotte et Ile-de-France, et La Réunion pour les autres populations qu'ES)
#usage_prediction_humain <- usage_prediction_humain %>%
#  filter(region != "Guyane") %>%
#  filter(region != "Ile-de-France") %>%
#  filter(region != "Mayotte") %>%
#  filter(region != "La Réunion" | population != "ville") %>%
#  filter(region != "La Réunion" | population != "EHPAD")
```
...
AMR_nationale %>%
  ...
  select(famille, annee, `effectif AMR`) %>%
...
ggplot(AMR_perte_regionale_animal, aes(x = reorder(population, perte), y = perte)) +
  ...
  labs(title = "Perte d'effectif lors du passage de l'échelle nationale à régionale",
       ...
       caption = str_glue("Données : Populations animales\nNote de lecture : Entre 2012 et 2021, la moitié des années ont fait face à une perte régionale d'au moins 50% chez les porcs."))+
...
=======
       caption = str_glue("Cohorte : population animale au niveau national\nLecture : En moyenne, 13.2% des animaux étudiés n’ont pas de profil de pan-sensibilité calculable.\nEn rouge, on observe la moyenne au sein de chaque population."))+
  scale_y_continuous(labels =scales::number_format(suffix=" %"),limits=c(0,100))+
  scale_x_discrete(labels = label)+
  annotate("text", x = 1.5, y = 18.5, label = "Moyenne = 13.2%", colour = "darkgreen", size = 3.3)
>>>>>>> 1279c98ce228c84600bb40f7aacc3e36316d8c37
```
...
=======
  annotate("text", x = 0.7, y = 4, label = paste0("Moyenne=", round(100-moyenne, 1), "%"), colour = "darkgreen", size = 3.2)
>>>>>>> 1279c98ce228c84600bb40f7aacc3e36316d8c37
```
...
#### 0 : descripteurs usuels de variabilité (médiane, écart inter-quartiles, variance, écart-type,coefficient de variation)
```{r}
# Calculer les descripteurs de variabilité pour chaque groupe
resultats <- AMR %>%
  group_by(region) %>%  # Grouper les données par la variable population
  summarise(
    mediane = median(`valeur AMR`),  # Calcul de la médiane
    q1 = quantile(`valeur AMR`, 0.25),  # Calcul du premier quartile
    q3 = quantile(`valeur AMR`, 0.75),  # Calcul du troisième quartile
    iqr = q3 - q1,  # Calcul de l'écart inter-quartiles (IQR)
    variance = var(`valeur AMR`),  # Calcul de la variance
    ecart_type = sd(`valeur AMR`),  # Calcul de l'écart-type
    coefficient_variation = (ecart_type / mean(`valeur AMR`)) * 100  # Calcul du coefficient de variation
  )
...
ggplot(AMR_humaine, aes(x = `valeur AMR`, y = region)) +
  ...
  theme_minimal()
```
Les variances régionales de la pan-S sont significativements différentes pour les populations animales mais par pour les humains (EHPAD et ES hemo). C'est assez contraire à ce que nous disais Mme Chauvin.
...
carteAMR1_Guad<-R1_Guad%>%
  ...
  labs(title = "Guadeloupe",
       caption = str_glue(paste("Données : population en ville en France métropolitaine et Outre-mer dont les données sont disponibles.\nNote de lecture : En 2021, la population en ville a une pan-sensibilité comprise entre ",min,"% et",max,"%.")))+
...
# Créer la mise en page en spécifiant les proportions
layout <- plot_layout(
  nrow = 3,
  heights = c(sizes, 0.1),
  widths = c(1)  # Tous les graphiques auront la même largeur
)

# Assembler les graphiques avec la mise en page
final_plot <- carteAMR1 / (carteAMR1_Mart + plot_spacer() + carteAMR1_Guad) / layout
...
carteAMR1<-R1%>%
  ...
  labs(title = "Pan-sensibilité de la population bovine en France en 2021",
       fill = "Pan-sensibilité",
       caption = str_glue(paste("Données : les bovins en France métropolitaine (des régions (en gris et Outre-mer) indisponibles).\nNote de lecture : La Normandie présente la pan-sensibilité la plus élevée chez les bovins : (",max(R1$`valeur AMR`,na.rm=TRUE),").")))+
  scale_fill_gradient(name = "taux de pan-sensibilité", limits = bornes,low='darkblue',high='lightblue')
carteAMR1_Guad<-R1_Guad%>%
  ggplot() +
  geom_sf(aes(geometry=geometry,fill=`valeur AMR`))+
...
=======
       caption = str_glue(paste("Données : La population des bovins en France métropolitaine\n(les données d'Outres-Mer et certaines régions (en gris) ne sont pas disponibles) - Anses\nNote de lecture : La population chien & chat de l'Auverge-Rhône-Alpes présente le taux de\npan-sensibilité le plus faible (",min(R1$`valeur AMR`,na.rm=TRUE),"),et donc la résistance la plus élevée, pour l'année 2021.")))+

       caption = str_glue(paste("Données : La population des bovins en France métropolitaine (des régions (en gris et Outre-mer) indisponibles).\nNote de lecture : En Auverge-Rhône-Alpes le taux de pan-sensibilité des chiens et chats est le plus bas :", min(R1$`valeur AMR`,na.rm=TRUE), "% en 2021.")))+
...
model_annee <- lm(`valeur AMR` ~ annee + region, data = AMR_regionale)
# Génération du qqplot avec les résidus du modèle
qqplot <- ggpubr::ggqqplot(residuals(model_annee))

# Ajout d'un titre et d'une note de lecture
qqplot +
  ggtitle("Diagramme Quantile-Quantile") +  # Ajout du titre
  xlab("Quantiles théoriques") +  # Ajout de l'axe x
  ylab("Quantiles empiriques") +  # Ajout de l'axe y
  labs(caption = str_glue("Données : populations régionales\nNote de lecture : La bissectrice représente les quantiles de la loi normale.")) +
  theme_minimal()  # Choix d'un thème pour améliorer l'esthétique du graphique


## 3 - Hypothèse d’homogénéité des variances
# Hypothèse vérifiée pour la variable annee (vu plus tôt) et pour la variable region (lorsqu'on prend chaque population indépendamment). Les résultats devront être interprétés par conséquent. De même, l'hypothèse d'homogénéité des variances n'est pas vérifiée pour des populations différentes --> chaque population indépendante. 
################## 1 test par population pour les régions
...
###### on calculemaintenant la part de la variance associée à chaque variable, pour chaque population.
AMR_regionale <- subset(AMR_regionale, population != "porc-abattoir")

anova_results <- list()

for (pop_level in unique(AMR_regionale$population)) {
  # Sous-ensemble de données pour le niveau de population actuel
  subset_data <- subset(AMR_regionale, population == pop_level)
  anova_results[[pop_level]] <- anova_test(lm(`valeur AMR` ~ region + annee, data =subset_data))
}

# Afficher les résultats pour chaque niveau de population
for (i in 1:length(anova_results)) {
  cat("Population:", unique(AMR_regionale$population)[i], "\n")
  print(anova_results[[i]])
}

anova_test(`valeur AMR` ~ population + region + annee, data = AMR_regionale)
...
qqplot +
  ...
  xlab("Quantiles théoriques") +  # Ajout de l'axe x
  ylab("Quantiles empiriques") +  # Ajout de l'axe y
  labs(caption = str_glue("Données : populations régionales - Anses\nNote de lecture : La bissectrice représente les quantiles de la loi normale.")) +
  theme_minimal()  # Choix d'un thème pour améliorer l'esthétique du graphique
```
...
ggplot(AMR_nationale_animal[which(AMR_nationale_animal$famille2 == "animale-abattoir"),], aes(x = annee, y = `valeur AMR`, group = population, color = population)) +
  ...
  labs(title = "Hausse générale de la sensibilité des populations issues d'abattoir",
       x = NULL,
       y = "Pan-sensibilité moyenne",
       color = "Population",
       caption = str_glue("Données : espèces animales issues d'abattoir - Anses\nNote de lecture : De 2019 à 2021, les bactéries étudiées sur les veaux ont démontré\n plus de résistance aux antibiotiques que les années précédentes.")) +
  theme_minimal()+
  scale_color_discrete(labels = c("Dinde", "Porc", "Poulet", "Veau", "Volaille"))
...
ggplot(AMR_nationale_humain, aes(x = annee, y = `valeur AMR`, group = population, color = population)) +
  ...
  labs(title = "Augmentation de la sensibilité aux antibiotiques des populations humaines",
       ...
       y = "Pan-sensibilité moyenne",
       color = "Population",
       caption = str_glue("Données : populations humaines - Anses\nNote de lecture : Les personnes en EHPAD sont 1,07% plus sensibles en 2021 qu'en 2012.")) +
  theme_minimal()+
  scale_color_discrete(labels = c("EHPAD", "ES", "ES-hemoculture", "Ville"))
```
...
Intégrer les poids dans le calcul ??
...
ggplot(data = usage_animal[which(usage_animal$population %in% c("Bovin", "Chats et chiens", "Chevaux", "Lapins", "Porcs", "Volailles")),], aes(x = `valeur AMR`, y = `valeur AMU`))+
  geom_path(size = 0.8)+
...
hist(usage_humain$`valeur AMR`[which(usage_humain$population == "EHPAD")], freq = FALSE, breaks = 30, main = "Les pan-S des populations d'EHPAD semblent suivre une loi bêta", xlim = c(0, 1), xlab = "Pan-S",ylab="Densité")
title(sub = "\nDonnées : les populations en EHPAD - Anses\nIndication : Bêta(a, b) où a et b sont calculés à partir de la moyenne et de la variance empirique", cex.sub = 0.8)
lines(density(rbeta(100, 31.471, 40.324)), col = 'darkgreen', lwd = 3)
...
#  filter(annee != 2021 | region != "Guadeloupe") %>%
#  filter(annee != 2019 | region != "Guadeloupe" | population == "ville")

for (pop in unique(usage_humain$population)){
  usage_filtre <- usage_humain[which(usage_humain$population == pop),]
  model_beta <- betareg(data = usage_filtre, `valeur AMR` ~ log(`valeur AMU`), weights = nrow(usage_filtre) * `effectif AMR` / sum(`effectif AMR`), link = "logit") # On prend le log car on a des données d'usage très dispersées (entre 1 et 300), et pas plus petites que 1, sinon le log devient négatif. Si on met région dans la régression plutôt qu'année ça donne des estimations pas significatives pour les coefficients de la valeur d'usage. On privilégie donc année qui donne des choses significatives.
  print(pop)
  print(summary(model_beta))
  print(AIC(model_beta))
}
...
On compare plusieurs modèles à l'aide de leur AIC (les epsilon dépendent des effectifs testés) : 
1 - logit(pan-S) = alpha + beta * AMU + epsilon 
2 - logit(pan-S) = alpha + beta * AMU + annee + epsilon
3 - logit(pan-S) = alpha + beta * AMU + region + epsilon
4 - logit(pan-S) = alpha + beta * AMU + region + annee + epsilon
Le modèle 1 semble mieux que le modèle 2 pour les animaux (année que significative pour les bovins), effectivement, les populations dites d'abattoir n'ont alors plus de coefficients estimés significatifs.
Le modèle 2 semble mieux que le modèle 1 pour les humains. 
Le modèle 3 semble mieux que le modèle 2 selon le critère de l'AIC pour les humains, cependant les coefficients obtenus sont plus significatifs dans le modèle 2, nous conservons le modèle 2.
Le modèle 4 n'est pas assez significatif pour les humains.
Une fois le modèle choisit, nous comparons le modèle de base avec celui du log : 
5 - logit(pan-S) = alpha + beta * log(AMU) + ...
Le modèle 1 est préférable au modèle 5 pour les populations animales.
Le modèle 5 est plus intéressant au modèle 2 pour les populations humains car les coefficients estimés sont moins proches de 0.

Les résidus des modèles sont assez dispersés pour valider l'hypothèse d'utilisation d'une régression bêta.

### Robustesse de l'indicateur

```{r}
mes_predictions_animaux <- as.data.frame(NULL)
for (pop in unique(usage_animal$population)){
  usage_filtre <- usage_animal[which(usage_animal$population == pop),]
  usage_prediction_filtre <- usage_prediction_animal[which(usage_prediction_animal$population == pop), c(2, 3)]
  model_beta <- betareg(data = usage_filtre, `valeur AMR` ~ `valeur AMU`, weights = nrow(usage_filtre) * `effectif AMR` / sum(`effectif AMR`), link = "logit")
  predictions <- predict(model_beta, newdata = usage_prediction_filtre, interval = "prediction")
  predictions_IC <- predict(model_beta, newdata = usage_prediction_filtre, interval = "prediction", type = "quantile", at = c(0.05, 0.95))
  table_prediction <- cbind(pop, predictions * 100, predictions_IC * 100)
  mes_predictions_animaux <- rbind(mes_predictions_animaux, table_prediction)
}

mes_predictions_humains <- as.data.frame(NULL)
for (pop in unique(usage_humain$population)){
  usage_filtre <- usage_humain[which(usage_humain$population == pop),]
  usage_prediction_filtre <- usage_prediction_humain[which(usage_prediction_humain$population == pop), c(2, 4, 3)]
  model_beta <- betareg(data = usage_filtre, `valeur AMR` ~ log(`valeur AMU`) + annee, weights = nrow(usage_filtre) * `effectif AMR` / sum(`effectif AMR`), link = "logit")
  predictions <- predict(model_beta, newdata = usage_prediction_filtre, se.fit=TRUE, interval = "prediction", level = 0.99)
  predictions_IC <- predict(model_beta, newdata = usage_prediction_filtre, interval = "prediction", type = "quantile", at = c(0.05, 0.95))
  table_prediction <- cbind(pop, unique(usage_prediction_filtre$region), predictions * 100, predictions_IC * 100)
  mes_predictions_humains <- rbind(mes_predictions_humains, table_prediction)
}
```

Une estimation par région pour les populations humains.

Pour étudier l'efficacité de cette prédiction, nous pouvons utiliser l'écart moyen des valeurs estimées avec les valeurs attendues.

```{r}
mes_predictions_humains <- mes_predictions_humains %>%
  arrange(pop, V2)

mes_predictions_humains <- cbind(mes_predictions_humains, usage_prediction_humain$`valeur AMR`)
colnames(mes_predictions_humains) <- c("population", "region", "estimation", "borne_inf", "borne_sup", "vraie valeur")

mes_predictions_humains$estimation <- as.numeric(mes_predictions_humains$estimation)
mes_predictions_humains$borne_inf <- as.numeric(mes_predictions_humains$borne_inf)
mes_predictions_humains$borne_sup <- as.numeric(mes_predictions_humains$borne_sup)

mes_predictions_animaux <- mes_predictions_animaux %>%
  arrange(pop)

mes_predictions_animaux <- cbind(mes_predictions_animaux, usage_prediction_animal$`valeur AMR`)
colnames(mes_predictions_animaux) <- c("population", "estimation", "borne_inf", "borne_sup", "vraie valeur")
mes_predictions_animaux$estimation <- as.numeric(mes_predictions_animaux$estimation)
mes_predictions_animaux$borne_inf <- as.numeric(mes_predictions_animaux$borne_inf)
mes_predictions_animaux$borne_sup <- as.numeric(mes_predictions_animaux$borne_sup)

ggplot(data = mes_predictions_humains, aes(x = estimation, y = `vraie valeur`))+
  geom_ribbon(aes(ymin = borne_inf, ymax = borne_sup), fill = "darkgreen", alpha = 0.1) +
  geom_point()+
  geom_abline(color = "darkgreen")+
<<<<<<< HEAD
  labs(title = "Les prédictions des pan-S humaines pour 2022 et leur intervalle de confiance",
       caption = "Données : Pan-S des humains en 2022 par région.\n Note de lecture : L'intervalle de confiance des prédictions est calculé au niveau 95%.",
=======
  labs(title = "Les prédictions des pan-S humaines pour 2022 et l'intervalle de confiance associé",
       subtitle = "Intervalle de confiance calculé à 95%",
       caption = "Données : Pan-S des humains en 2022 par région - Anses\n Note de lecture : La majorité des estimations se trouvent proches des vraies valeurs.",
>>>>>>> 1279c98ce228c84600bb40f7aacc3e36316d8c37
       x = "Pan-S estimée",
       y = "Pan-S réelle")+
...
ggplot(data = mes_predictions_animaux, aes(x = estimation, y = `vraie valeur`))+
  geom_ribbon(aes(ymin = borne_inf, ymax = borne_sup), fill = "darkgreen", alpha = 0.1) +
  geom_point()+
  geom_abline(color = "darkgreen")+
  labs(title = "Les prédictions des pan-S animales pour 2022 et leur intervalle de confiance",
       subtitle = "Intervalle de confiance calculé à 95%",
<<<<<<< HEAD
       caption = "Données : Pan-S des animaux en 2022 en France entière.\n Note de lecture : Seules 3 observations se trouvent dans l'intervalle de confiance des prédictions.",
=======
       caption = "Données : Pan-S des animaux en 2022 en France entière - Anses\n Note de lecture : Seules 2 observations se trouvent dans l'intervalle de confiance des prédictions.",
>>>>>>> 1279c98ce228c84600bb40f7aacc3e36316d8c37
       x = "Pan-S estimée",
       y = "Pan-S réelle")+
  scale_y_continuous(labels =scales::number_format(suffix=" %"))+
...
for (pop in unique(mes_predictions_humains$population)){
  print(pop)
  print(rmse(mes_predictions_humains$`vraie valeur`[which(mes_predictions_humains$population == pop)], mes_predictions_humains$estimation[which(mes_predictions_humains$population == pop)]))
}

# Racine de l'erreur quadratique moyenne
for (pop in unique(mes_predictions_animaux$population)){
  print(pop)
  print(rmse(mes_predictions_animaux$`vraie valeur`[which(mes_predictions_animaux$population == pop)], mes_predictions_animaux$estimation[which(mes_predictions_animaux$population == pop)]))
}
```

# Graphique pour le diapo de l'oral
```{r}
# Fusionner les données AMR et les prévisions par date
graphique_oral <- usage_animal[,c(4,5,8)]
graphique_oral$`valeur AMR` <- graphique_oral$`valeur AMR`*100
graphique_oral$borne_inf <- graphique_oral$`valeur AMR`
graphique_oral$borne_sup <- graphique_oral$`valeur AMR`
mes_predictions_animaux$annee <- 2022
colnames(mes_predictions_animaux) <- c("population", "valeur AMR", "borne_inf", "borne_sup", "delad", "delad2", "annee")
donnees_fusionnees <- rbind(graphique_oral, mes_predictions_animaux[,-c(5, 6)])
donnees_fusionnees$annee <- as.numeric(donnees_fusionnees$annee)

pour_oral <- donnees_fusionnees[c(1:10, 15:54, 61:70, 77, 79:82, 84),]
vraies_valeurs$AMR <- mes_predictions_animaux$`vraie valeur`

# Créer le graphique ggplot
ggplot(pour_oral, aes(x = annee, color = population, fill = population)) +
  geom_ribbon(aes(ymin = borne_inf, ymax = borne_sup), color = "transparent", alpha = 0.3) +
  geom_line(aes(y = `valeur AMR`))+
  labs(title = "Prédictions et IC (à 95%) des Pan-S animales pour 2022",
       x = NULL,
       y = "Pan-S",
       colour = "Population",
       fill = "Population",
       caption = "Données : espèces cliniques animales en France entière - Anses.")+
...
```{r}
UA<-usage_animal%>%
  select(famille,population,annee,region,`valeur AMU`,`valeur AMR`,`effectif AMR`)
UA<-usage_animal[c(1,24,32,45,48),]%>%
    select(famille,population,annee,region,`valeur AMU`,`valeur AMR`,`effectif AMR`)

usage<-rbind(usage_humain[1,],usage_humain[88,],usage_humain[124,],usage_humain[180,])
usage<-usage%>%
  mutate(famille="humaine")%>%
    select(famille,population,annee,region,`valeur AMU`,`valeur AMR`,`effectif AMR`)

rbind(UA,usage)

xtable(rbind(UA,usage))
names(usage)
