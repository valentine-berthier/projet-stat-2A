...
Des regroupements entre populations animales ont été effectués dans ce jeu de données : 
...
lapin = Lapins
La pan-sensibilité est alors calculée en prenant en compte la biomasse de chaque espèce. Les calculs ont déjà été fait pour les groupes de population, mais seront peut-être à expliquer dans le rapport.
...
```
Il n'y a effectivement pas de valeurs proches de la borne supérieure, les valeurs se regroupent entre 0,1 et 0,7. De plus, pour chaque population, la majorité des valeurs de la variable (75%) se trouvent dans un intervalle plus petit que 0,1 et si on ne découpe pas par population, 75% des valeurs se trouvent dans l'intervalle [0,3; 0,6]. On peut conclure quant à l'hétéroscédasticité de la variable.
...
print le qqplot pour voir si la normalité des données est bien observée.

##### Lien avec l'usage de l'année n-1 plutôt qu'avec l'année n

```{r}
# On crée le nouveau data frame où l'usage de l'année n-1 se trouve en face de la pan-S de l'année n
usage_animal <- usage_animal %>%
  arrange(population, annee)

usage_animal2 <- data.frame(matrix(vector(), 0, 9, dimnames=list(c(), colnames(usage_animal))))

for (pop in unique(usage_animal$population)){
  don_pop <- usage_animal[which(usage_animal$population == pop),]
  for (ligne in 0:(nrow(don_pop)-2)){
    don_pop$`valeur AMU`[nrow(don_pop) - ligne] <- don_pop$`valeur AMU`[nrow(don_pop) - ligne - 1]
  }
  usage_animal2 <- rbind(usage_animal2,don_pop[2:nrow(don_pop),])
}

for (pop in unique(usage_animal2$population)){
  usage_filtre <- usage_animal2[which(usage_animal2$population == pop),]
  model_beta <- betareg(data = usage_filtre, `valeur AMR` ~ `valeur AMU`, link = "logit")
  print(pop)
  print(summary(model_beta))
}
...
```{r}
# Il faut récupérer les données de pan-sensibilité du fichier données_AMR
usage_humain <- usage %>%
  filter(famille == "Humaine") %>%
...
usage_humain$population <- as.factor(usage_humain$population)
usage_humain$region <- as.factor(usage_humain$region)
```

```{r}
# Dans la régression bêta, chaque variable doit être une proportion, donc doit avoir des valeurs comprises entre 0 et 1. Or la valeur de résistance est comprise entre 0 et 100. De plus, on ne garde que les observations pour lesquelles la valeur de pan-sensibilité est renseignée.
...
Nous observons des données hétéroscédastiques par population, en EHPAD la variance est bien plus grande qu'en ville. Il sera possible de tester ce critère à l'aide d'un test d'homoscédasticité. De plus, on ne remarque pas de données proche des bornes de l'intervalle, cela justifie également le fait d'utiliser une régression bêta.
...
Il faudra voir pourquoi on mets du log pour les valeurs AMU et peut-être qu'il faudrait ajouter les régions à l'analyse car elles influent également sur les résultats (mais pour le moment j'ai une erreur si j'ajoute région...). On peut tester la significativité des coefficients car je les trouve très proches de 0.
...
# Hypothèse de normalité
shapiro_test(residuals(modele_panel)) #p-value > 0,05 donc on rejette l'hypothèse nulle, ainsi l'hypothèse de normalité des résidus est vérifiée.

#### Test hetéroscédasticité
# Ajuster votre modèle de données de panel
modele <- plm(usage_animal$`valeur AMR` ~ Valeur_AMU + annee, data = usage_animal, model = "pooling")

# Obtenir les résidus
residus <- resid(modele)

# Ajuster la régression auxiliaire
reg_aux <- lm(residus^2 ~ usage_animal$Valeur_AMU + usage_animal$annee)
if(!require("lmtest")){
  install.packages("lmtest")
  require("lmtest")
}
# Effectuer le test de Breusch-Pagan
bp_test <- bptest(reg_aux)
# C'EST HETEROSCEDASTIQUE...
```
...
Cependant après une analyse les données semblent hétéroscédastique, on va donc privilégier une regression bêta.
...
```{r}
# Chargement de la bibliothèque requise
library(rpart)
library(rpart.plot)

data_arbre_cart <- AMR_animale %>%
  select(`valeur AMR`, population, annee, region) %>%
  mutate(population = as.factor(population), annee = as.factor(annee), region = as.factor(region))

# Construction de l'arbre de décision
arbre_decision <- rpart(`valeur AMR` ~ population + region, data = data_arbre_cart)

# Affichage de l'arbre
print(arbre_decision)

# Visualisation graphique de l'arbre avec plus de branches
...
```{r}
data_glm <- AMR_animale %>%
  select(`valeur AMR`, population, annee, region, famille2) %>%
  mutate(population = as.factor(population), annee = as.factor(annee), region = as.factor(region))

fit <- glm(data = data_glm, `valeur AMR` ~ population)
summary(fit)

# HO : "GLM non significatif" -> beta1 = 0 vs H1 : "GLM significatif"
qchisq(p=0.90,df=16)
fit$null.deviance - fit$deviance

data_glm <- AMR_humaine %>%
  select(`valeur AMR`, population, annee, region) %>%
  mutate(population = as.factor(population), annee = as.factor(annee), region = as.factor(region))
...
summary(fit)
```
Clairement ici, le GLM semble significatif car deviance nulle - deviance GLM >> quantile d'un khi2
Bovin adulte est la modalité de référence.