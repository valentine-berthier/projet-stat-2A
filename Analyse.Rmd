...
### Test d'Hausmann : on regarde si on utilise les effets fixes ou aléatoires
if(!require("plm")){
  install.packages("plm")
  require("plm")
}

# Ajuster un modèle à effets fixes
modele_fixe <- plm(usage_animal$`valeur AMR` ~ Valeur_AMU + annee, data = usage_animal, model = "within")

# Ajuster un modèle à effets aléatoires
modele_aleatoire <- plm(usage_animal$`valeur AMR` ~ Valeur_AMU + annee, data = usage_animal, model = "random")

# Extraire les résidus des deux modèles
residus_fixes <- resid(modele_fixe)
residus_aleatoires <- resid(modele_aleatoire)

df <- 2

# Calculer la statistique de test d'Hausman
hausman_stat <- sum(residus_fixes^2 - residus_aleatoires^2)
```
...
```{r}
# Installe le package s'il n'existe pas encorre sinon ne l'installe pas
if(!require("bit64")){
