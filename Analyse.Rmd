...
AMR_perte_regionale <- AMR_perte_regionale %>%
  mutate(perte = effectif_regional/effectif_national) %>%
  select(annee, population, perte)

for (pop in unique(AMR_perte_regionale$population)){
  ggplot(AMR_perte_regionale, aes(x = annee, y = perte, , group = population, color = population))+
    geom_line(stat = "summary", fun = "mean") +
    geom_point(stat = "summary", fun = "mean", size = 2)
}
```
