...
# C'EST HETEROSCEDASTIQUE BORDEL

#### On fait donc un GLM youpi
# Ajuster un modèle GLM avec une structure de variance spécifiée (par exemple, Gamma ou Gaussian)
modele_glm <- glm(`valeur AMR` ~ Valeur_AMU + annee, data = usage_animal, family = gaussian)

# Obtenir un aperçu du modèle
summary(modele_glm)

# Visualiser les résidus
plot(modele_glm, which = 1)  # Visualisation des résidus standardisés
plot(modele_glm, which = 2)  # Visualisation des résidus quantiles

# Si nécessaire, ajuster un modèle GLM avec une structure de variance spécifiée (par exemple, Gamma ou Inverse Gaussian)
# modele_glm2 <- glm(valeur_AMR ~ Valeur_AMU + annee, data = usage_animal, family = gamma)
# summary(modele_glm2)
