...
# Analyse variance indépendante
manova_result <- aov(`valeur AMR` ~ annee, data = data_anova)
modele_anova <- aov(`valeur AMR` ~ region, data = data_anova)
pop_anova <- aov(`valeur AMR` ~ population, data = data_anova)
plot(pop_anova)
manova_result
modele_anova

# NE PAS OUBLIER DE VERIFIER LES HYPOTHESES (normalité, pas de valeur aberrante, indépendance, homoscédasticité)
# Analyse conjointe
data_anova <- data_anova[-which(AMR_humaine$region=='Nationale'),] # Si on veut supprimer le national
