...
# TEST DE TENDANCE (FAUX)
```{r}
#Testons le résultat précédent :
"""
library(lmtest)
library(sandwich)

# Diviser les données en sous-groupes en fonction de la variable de population
sous_groupes <- split(AMR_animale_fr, AMR_animale_fr$population)

# Réaliser un test de tendance pour chaque sous-groupe
resultats_test_tendance <- lapply(sous_groupes, function(sous_groupe) {
  modele <- lm(`valeur AMR` ~ annee, data = sous_groupe)
  test_tendance <- coeftest(modele, vcov = vcovHC)
  return(test_tendance)
})

#resultats_test_tendance

# Pour les humains
sous_groupes_humain <- split(AMR_humaine_fr, AMR_humaine_fr$population)

resultats_test_tendance_humain <- lapply(sous_groupes_humain, function(sous_groupe) {
  modele <- lm(`valeur AMR` ~ annee, data = sous_groupe)
  test_tendance <- coeftest(modele, vcov = vcovHC)
  return(test_tendance)
})

#resultats_test_tendance_humain

################# Modèle linéaire généralisé à effets mixtes
# Un test de McNemar ne s'effectue que sur 2 proportions, hors ici nous possédons plus de 2 années par population. De plus, ne prendre que l'année de départ et d'arrivée omettrait les informations contenues dans les autres années ce qui créérait du biais.
# Nous allons donc réaliser un modèle linéaire généralisé à effet mixte.
library(lme4)
# Liste pour stocker les résumés des modèles GLM pour chaque population
liste_resumes_modeles_glm <- list()

# Boucle pour ajuster un modèle GLM pour chaque population et stocker les résumés
for (pop in unique(AMR_animale_fr$population)) {
  # Sous-ensemble des données pour la population actuelle
  donnees_population <- subset(AMR_animale_fr, population == pop)
"""

  # Ajuster un modèle GLM pour la population actuelle
'''  
modele_glm <- glm(`valeur AMR` ~ annee, family = Gamma(link = "log"), data = donnees_population)
  
  # Obtenir le résumé du modèle
  resume_modele <- summary(modele_glm)
  
  # Stocker le résumé dans la liste
  liste_resumes_modeles_glm[[pop]] <- resume_modele
}

# Afficher les résultats des tests pour chaque population
for (i in seq_along(liste_resumes_modeles_glm)) {
  cat("Population:", names(liste_resumes_modeles_glm)[i], "\n")
  print(liste_resumes_modeles_glm[[i]])
  cat("\n")
}'''
...
AMR_regionale <- AMR[which(AMR$region != "Nationale"),]

effectif_regional <- aggregate(AMR_regionale$`effectif AMR`, by = list(AMR_regionale$annee, AMR_regionale$population), FUN = sum)
colnames(effectif_regional) <- c("annee", "population", "effectif_regional")

AMR_perte_regionale <- inner_join(AMR_perte_regionale, effectif_regional)

for (pop in unique(AMR_perte_regionale$population)){
  ggplot(AMR_perte_regionale)
}
```
...
for (pop in unique(usage_animal$population)){
  ...
  print(pop)
  print(model_beta$coefficients$mean)
}
```
