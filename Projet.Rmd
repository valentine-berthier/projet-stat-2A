...
donnees_remodelees <- AMR_regionale %>%
  mutate(annee = case_when(
    ...
    annee %in% c(2018, 2019) ~ "2018-2019",
    TRUE ~ "2020-2021")) %>%
  tidyr::pivot_wider(names_from = population, values_from = `valeur AMR`, id_cols = c(region, annee))

donnees_remodelees <- as.data.frame(donnees_remodelees)

for (i in 1:nrow(donnees_remodelees)) {
  for (j in 3:ncol(donnees_remodelees)) {
    donnees_remodelees[i, j] <- mean(unlist(donnees_remodelees[i, j]))
    donnees_remodelees[i, j] <- unlist(donnees_remodelees[i, j])
  }
}
...
donnees_remodelees <- donnees_remodelees %>%
  arrange(annee)
```
Il y a pleins de warning mais pour le moment je n'arrive pas à faire autrement. D'abord on concatène les années, ensuite on transpose la table pour avoir les populations en colonne. Ensuite ça a formé un tibble à la place d'un data frame donc je remets en data frame. Puis, je calcule les moyennes car les données sont en doubles depuis que j'ai fusionné les années par deux.

```{r}
matrice_correlations <- cor(unlist(donnees_remodelees[, -c(1,2)]), use = "pairwise.complete.obs")

# Afficher le corrplot
corrplot::corrplot(matrice_correlations, method = "color", addrect = 2, addCoef.col = "black")
```
