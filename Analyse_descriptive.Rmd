...
Similarité entre les populations possédant des moyennes proches ? (chat et chien qui vivent plus en solitaires --> moyenne parmis les plus élevées)
...
Pour poursuivre l'analyse :
Similarités entre les population possédant des variances intra proches ?
...
ggplot(AMR_animale_fr_horsabattoir, aes(x = annee, y = `valeur AMR`, group = population, color = population)) +
  ...
  theme_minimal()
```
On note une tendance croissante de l'AMR en fonction des années pour les populations en abattoir. Pour les autres populations, la tendance est beaucoup moins claire --> la variation intra n'est pas dûe à une tendance croissante (ou décroissante) mais à un autre facteur !
