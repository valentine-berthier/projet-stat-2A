...
## Modèle Resapath 
```{r Idee 1, echo=FALSE, include=TRUE}
...
etude_effectif<-function(modele,pop){
table1<-modele%>%
  filter(profil_AMR=="pan-sensible")%>%
  mutate(eff_pansensible=effectif_AMR)%>%
  select(annee,population,eff_pansensible)
table2<-modele%>%
  filter(profil_AMR=="C3G-R")%>%
  mutate(eff_C3G=effectif_AMR)%>%
  select(annee,population,effectif_AMR)

# Jointure des tables sur les variables var1 et var2
J <- merge(table1, table2, by = c(annee, population), all = TRUE)

plot<-J%>%
  ggplot(data=modele, aes(x =population, y = eff_pansensible/eff_C3G),ylim=c(0,1000)) +
  geom_col(width = 1, aes(fill =profil_AMR)) +       # tracer le nombre de cas sous forme de colonnes
  theme_minimal()+                              # simplifier les  arrière-plans
  labs(                                         # ajouter  les noms d'axes, titres ... 
    x = "Profil AMR ",
...
pop<-"veau"
#etude_effectif(ResapathFrance_sansSARM,pop)
#for (pop in levels(ResapathFrance_sansSARM$population)){
#  plot(etude_effectifmoy_profil(Modele_LNR_sansSARM,pop))
#  }


```
```{r Bonne Idee, echo=FALSE, include=TRUE}
...
for (pop in levels(J$population)){
  J1<-J%>%
    filter(population==pop)%>%
  ...
  moyenne=round(mean(J1$taux_eff),2)
  
  plot<-ggplot(data=J1, aes(x =annee, y = taux_eff,fill=min_max_value))+
    geom_col(width = 0.5) +       # tracer le nombre de cas sous forme de colonnes
    theme_minimal()+                              # simplifier les  arrière-plans
    labs(                                         # ajouter  les noms d'axes, titres ... 
      x = "annee ",
      y = "taux de sensibilité à tous les tests par rapport à C3G-R",
      title = "taux de sensibilité par population étudiées en fonction de l'année étudiée",
...
for (pop in levels(ResapathFrance_sansSARM$population)){
  tab<-ResapathFrance_sansSARM %>% 
    filter(population==pop)%>%
    spread(annee,effectif_AMR)%>%
    xtabs(effectif_AMR ~ profil_AMR + annee, data = data.frame(annee,profil_AMR))
  
  print(tab)
}
colnames(ResapathFrance_sansSARM)



tableau_croise <- xtabs(ResapathFrance_sansSARM$effectif_AMR~ ResapathFrance_sansSARM$profil_AMR + ResapathFrance_sansSARM$annee, data = data.frame(ResapathFrance_sansSARM$annee, ResapathFrance_sansSARM$profil_AMR))
